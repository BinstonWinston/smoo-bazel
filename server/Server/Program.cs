...
using Server.GameMode.CaptureTheFlag;
using System.Linq;
...
server.PacketHandler = (c, p) => {
    switch (p) {
        case GamePacket gamePacket: {
            ...
            GameModeManager.updatePlayerStage(c.Id, gamePacket.ScenarioNum, gamePacket.Stage);
...
#pragma warning disable CS4014
            server.Broadcast(playerPacket, c)
                .ContinueWith(x => { if (x.Exception != null) { consoleLogger.Error(x.Exception.ToString()); } });
#pragma warning restore CS4014
            GameModeManager.updatePlayerTransform(c.Id, playerPacket.Position, playerPacket.Rotation);
            return false;
        }
        case PlayerPacket playerPacket when Settings.Instance.Flip.Enabled
...
#pragma warning restore CS4014
        ...
        case FlagHoldStatePacket flagHoldStatePacket: {
#pragma warning disable CS4014
            server.Broadcast(flagHoldStatePacket, c)
                .ContinueWith(x => { if (x.Exception != null) { consoleLogger.Error(x.Exception.ToString()); } });
            c.Send(flagHoldStatePacket)
                .ContinueWith(x => { if (x.Exception != null) { consoleLogger.Error(x.Exception.ToString()); } });
#pragma warning restore CS4014
            var gameMode = gameModeManager.getGameMode();
            if (gameMode != null && gameMode is CaptureTheFlagMode) {
                var captureTheFlagMode = (CaptureTheFlagMode)gameMode;
                var flagState = new FlagState(
                    flagHoldStatePacket.Team, 
                    flagHoldStatePacket.IsHeld ? c.Id : null,
                    GameModeManager.getLastPosition(c.Id),
                    GameModeManager.getLastRotation(c.Id),
                    GameModeManager.getLastScenarioNum(c.Id),
                    GameModeManager.getLastStage(c.Id));
                captureTheFlagMode.updateFlagData(flagState);
            }
            return false;
        }
        case CaptureTheFlagWinPacket ctfWinPacket: {
            c.Logger.Info($"Winning team: {ctfWinPacket.WinningTeam}");
#pragma warning disable CS4014
            server.Broadcast(ctfWinPacket)
                .ContinueWith(x => { if (x.Exception != null) { consoleLogger.Error(x.Exception.ToString()); } });
#pragma warning restore CS4014
            return false;
        }
...
CommandHandler.RegisterCommand("ctf", args => {
    const string optionUsage =
        "Valid options:\n\tstart <time> <red_team>";
    if (args.Length < 3)
        return optionUsage;
    switch (args[0]) {
        case "start" when args.Length > 2: {
            if (!byte.TryParse(args[1], out byte time)) return $"Invalid countdown seconds {args[1]} (range: 0-255)";
            string[] redTeamNames = args[2..];
            Client[] redTeam = server.Clients.Where(c => redTeamNames.Contains(c.Name)).ToArray();
            if (redTeam.Length != redTeamNames.Length)
                return
                    $"Couldn't find player{(redTeamNames.Length > 1 ? "s" : "")}: {string.Join(", ", redTeamNames.Where(name => server.Clients.All(c => c.Name != name)))}";
            Client[] blueTeam = server.Clients.Except(redTeam).ToArray();
            String[] blueTeamNames = blueTeam.Select(c => c.Name).ToArray();

            Task.Run(async () => {
                int realTime = 1000 * time;
                await Task.Delay(realTime);
                gameModeManager.startGameMode(new CaptureTheFlagMode(redTeamNames, blueTeamNames, server));
                consoleLogger.Info($"Started game with red team members {string.Join(", ", redTeamNames)} and blue team members {string.Join(", ", blueTeamNames)}");
            });
            return $"Starting game in {time} seconds with red team members {string.Join(", ", redTeamNames)}";
        }
        default:
            return optionUsage;
    }
