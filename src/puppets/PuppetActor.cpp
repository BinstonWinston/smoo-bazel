...
void PuppetActor::control() { 
    if(mInfo) {
        ...
        if (mNameTag) {
            ...
            else if (GameModeManager::instance()->isModeAndActive(GameMode::CAPTURE_THE_FLAG)) {
                mNameTag->mIsAlive =
                    GameModeManager::instance()->getInfo<CaptureTheFlagInfo>()->mPlayerTeam == mInfo->ctfTeam;
            }
            else {
