...
void drawDebugMode(HakoniwaSequence *curSequence, sead::Viewport *viewport, sead::DrawContext *drawContext) {
    ...
    gTextWriter->printf("GameMode: %d\n", GameModeManager::instance()->getGameMode());

    {
        PlayerActorBase* playerBase = rs::getPlayerActor(curSequence->curScene);
        auto const p = al::getTrans(playerBase);
        gTextWriter->printf("Player pos: %.2f, %.2f, %.2f\n", p.x, p.y, p.z);
        auto const borderOrigin = al::getTrans(BorderActor::singleton);
        sead::Vector3f frontDir{};
        al::calcFrontDir(&frontDir, BorderActor::singleton);
        sead::Quatf quat = BorderActor::singleton->mPoseKeeper->getQuat();
        sead::Vector3f rpy{};
        quat.calcRPY(rpy);
        gTextWriter->printf("Border: P(%.2f, %.2f, %.2f), Dir(%.2f, %.2f, %.2f), RPY(%.2f, %.2f, %.2f)\n", 
            borderOrigin.x, borderOrigin.y, borderOrigin.z,
...

    {
        ...
        gTextWriter->printf("Stage Info: %s, %d\n",
            GameDataFunction::getCurrentStageName(stageScene->mHolder),
            stageScene->mHolder.mData->mGameDataFile->getScenarioNo());
    }
    ...
    if (GameModeManager::instance()->isMode(GameMode::HIDEANDSEEK)) {
        ...
        gTextWriter->printf("H&S IsIt: %d\n", static_cast<int>(curInfo->mIsPlayerIt));
        gTextWriter->printf("H&S Hiding Time Seconds: %d\n", static_cast<int>(curInfo->mHidingTime.mSeconds));
    }

    if (GameModeManager::instance()->isModeAndActive(GameMode::CAPTURE_THE_FLAG)) {
        auto* curInfo = GameModeManager::instance()->getInfo<CaptureTheFlagInfo>();
        gTextWriter->printf("CTF HeldFlag: %d\n", static_cast<int>(curInfo->mHeldFlag));
        gTextWriter->printf("CTF Received Held Flag Team: %d\n", static_cast<int>(FlagActor::latestReceivedFlagTeam));
        gTextWriter->printf("CTF Received Held Flag Pos: (%.2f, %.2f, %.2f)\n",
            FlagActor::latestReceivedPosition.x,
            FlagActor::latestReceivedPosition.y,
            FlagActor::latestReceivedPosition.z);
        auto redPos = al::getTrans(FlagActor::getFlagActorForTeam(FlagTeam::RED).value());
        auto bluePos = al::getTrans(FlagActor::getFlagActorForTeam(FlagTeam::BLUE).value());
        gTextWriter->printf("CTF Red Pos: (%.2f, %.2f, %.2f)\n", redPos.x, redPos.y, redPos.z);
        gTextWriter->printf("CTF Blue Pos: (%.2f, %.2f, %.2f)\n", bluePos.x, bluePos.y, bluePos.z);

        gTextWriter->printf("CTF Time: %02d:%02d\n", curInfo->mRoundTime.mMinutes, curInfo->mRoundTime.mSeconds);
        gTextWriter->printf("Flag Stage: %s\n", FlagActor::latestReceivedStage);
    }
...
void drawMainHook(HakoniwaSequence *curSequence, sead::Viewport *viewport, sead::DrawContext *drawContext) {
    ...
    if (debugMode) {
        drawDebugMode(curSequence, viewport, drawContext);
    }
