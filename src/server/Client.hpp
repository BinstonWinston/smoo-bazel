...
class Client {
    ...
    public:
        ...
        // Get the player transform for the given user ID. Returns nullopt if user ID can't be found or if the user is in a different stage.
        std::optional<std::tuple<sead::Vector3f, sead::Quatf>> getPlayerTransformInSameStage(nn::account::Uid const& userID);
